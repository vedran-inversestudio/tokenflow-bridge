...
let BRIDGE_URL = 'http://localhost:4000';
let BRIDGE_WS_URL = 'ws://localhost:4000';
    ...
    capabilities: {
      tools: {
        ...
        list: () => Promise.resolve([
          ...
          {
            name: 'configureBridgeConnection',
            description: 'Configure the bridge server connection URL',
            inputSchema: {
              type: 'object',
              properties: {
                bridgeUrl: {
                  type: 'string',
                  description: 'Bridge server URL (e.g., http://localhost:4000 or https://my-project-bridge.vercel.app)'
                },
                projectId: {
                  type: 'string',
                  description: 'Project ID for isolation (optional)'
                }
              },
              required: ['bridgeUrl']
            }
          ...
          {
            name: 'listProjects',
            description: 'List all available projects in the bridge server',
            inputSchema: {
              type: 'object',
              properties: {},
              required: []
...
function updateBridgeConnection(bridgeUrl, projectId = 'default') {
  const oldBridgeUrl = BRIDGE_URL;
  const oldProjectId = currentProjectId;
  
  // Update URLs
  BRIDGE_URL = bridgeUrl;
  BRIDGE_WS_URL = bridgeUrl.replace('http://', 'ws://').replace('https://', 'wss://');
  ...
  // Connect to new bridge server
  connectToBridge();
  
  console.log(`🔌 Updated bridge connection: ${oldBridgeUrl} (${oldProjectId}) → ${BRIDGE_URL} (${currentProjectId})`);
}
    ...
    bridgeWebSocket.on('message', (data) => {
      try {
        ...
        if (message.type === 'tokenData') {
          ...
          if (!message.projectId || message.projectId === currentProjectId) {
            lastTokenData = message.data;
            console.log('📦 Received token data update:', {
              projectId: message.projectId || currentProjectId,
              cleanTokenCount: lastTokenData.filtered ? Object.keys(lastTokenData.filtered.cleanTokens || {}).length : 0,
              timestamp: message.timestamp
            });
          }
        } else if (message.type === 'allProjectData') {
          // Handle multi-project data
          if (message.projects && message.projects[currentProjectId]) {
            lastTokenData = message.projects[currentProjectId].data;
            console.log('📦 Received multi-project token data update:', {
              projectId: currentProjectId,
              cleanTokenCount: lastTokenData.filtered ? Object.keys(lastTokenData.filtered.cleanTokens || {}).length : 0,
              timestamp: message.projects[currentProjectId].timestamp
            });
          }
        }
      } catch (error) {
...
// Get current token data from bridge server
  ...
  try {
    switch (name) {
      case 'getCurrentTokens': {
        const url = currentProjectId !== 'default' 
          ? `${BRIDGE_URL}/api/tokens?project=${currentProjectId}`
          : `${BRIDGE_URL}/api/tokens`;
          ...
          content: [
            {
              ...
              text: `✅ Retrieved current token data from bridge server.\n\n` +
                    ...
                    `- Project ID: ${data.projectId || currentProjectId}\n\n` +
      ...
      case 'configureBridgeConnection': {
        const { bridgeUrl, projectId = 'default' } = args;
        ...
        try {
          ...
          // Update connection
          updateBridgeConnection(bridgeUrl, projectId);
          
          return {
            content: [
              {
                type: 'text',
                text: `✅ Bridge connection configured successfully!\n\n` +
                      `**Bridge URL:** ${bridgeUrl}\n` +
                      `**Project ID:** ${projectId}\n` +
                      `**Status:** Connected\n\n` +
                      `The MCP server is now connected to the specified bridge server.\n` +
                      `Use \`getCurrentTokens\` to retrieve token data or \`listProjects\` to see available projects.`
              }
            ]
          };
        } catch (error) {
          return {
            content: [
              {
                type: 'text',
                text: `❌ Failed to configure bridge connection: ${error.message}\n\n` +
                      `Please ensure:\n` +
                      `• The bridge server is running at ${bridgeUrl}\n` +
      ...
      case 'listProjects': {
        try {
          ...
          if (data.projects.length === 0) {
            return {
              content: [
                {
                  type: 'text',
                  text: `📋 No projects found in bridge server.\n\n` +
                        `**Bridge URL:** ${BRIDGE_URL}\n` +
                        `**Status:** No data available\n\n` +
                        `Extract tokens from Figma first to create projects.`
                }
              ]
            };
          }
          ...
          const projectList = data.projects.map(project => 
            `• **${project.projectId}** (${project.tokenCount} tokens) - Last updated: ${project.lastUpdate}`
          ).join('\n');
          ...
          return {
            content: [
              {
                type: 'text',
                text: `📋 Available Projects (${data.totalProjects})\n\n` +
                      `**Bridge URL:** ${BRIDGE_URL}\n\n` +
                      projectList + '\n\n' +
                      `Use \`configureBridgeConnection\` to switch to a specific project.`
              }
            ]
          };
        } catch (error) {
